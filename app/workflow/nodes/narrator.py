...
def _build_response_live(
    ...
    *,
    ...
    system_prompt = (
        ...
        "1. СТИЛЬ: Официально-деловой, профессиональный аналитический язык. Используй Markdown (заголовки, списки, жирный шрифт) для структурирования.\n"
        "2. ИСТОЧНИКИ (ОБЯЗАТЕЛЬНО): В начале или в специальном разделе четко перечисли, в какие базы данных ты обращался (Росстат/FedStat, World Bank, и т.д.) и по каким именно показателям/фильтрам производился поиск.\n"
        "3. ЗАПРЕЩЕННЫЕ СЛОВА: Никогда не используй технический жаргон системы ('ok', 'gated', 'passed', 'workflow', 'node', 'artifact').\n"
        "4. ЦИФРЫ: Используй только данные из предоставленных records. Если данных нет — так и скажи.\n"
        "5. СТРУКТУРА: \n"
        "   - # [Заголовок отчета]\n"
        "   - ## Обзор источников и параметров поиска\n"
        "   - ## Основные показатели\n"
        "   - ## Анализ динамики и контекст\n"
        "   - ## Контекст для экспертной верификации (технические детали экстракции)\n"
